package study.com.feedbackdemo.imagepicker.util;
...
import study.com.feedbackdemo.imagepicker.StorageType;
...
class ExternalStorage {
    ...
    public void init(Context context, String sdkStorageRoot) {
        if (!TextUtils.isEmpty(sdkStorageRoot)) {  //如果不为null
            File dir = new File(sdkStorageRoot);
            if (!dir.exists()) { //如果dir不存在
                dir.mkdirs();
            }
            if (dir.exists() && !dir.isFile()) {  //如果dir存在，并且dir不是文件
                this.sdkStorageRoot = sdkStorageRoot;
                if (!sdkStorageRoot.endsWith("/")) {  //如果sdkStorageRoot的最后字符不等于“/”
                    this.sdkStorageRoot = sdkStorageRoot + "/";
                }
            }
        }

        if (TextUtils.isEmpty(this.sdkStorageRoot)) {  //如果外部存储根目录为空
    ...
    private void createSubFolders() {
        ...
        if (root.exists() && !root.isDirectory()) {  //如果存在和外部存储目录一样的文件（不是文件夹），那么将其删除
            root.delete();
        }
        for (StorageType storageType : StorageType.values()) {  //根据存储类型来获取将要创建文件的路径
            result &= makeDirectory(sdkStorageRoot + storageType.getStoragePath());
        }
        if (result) { //如果所创建的文件都成功，那么将创建没有媒介的文件
